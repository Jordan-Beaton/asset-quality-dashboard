...
- My Actions now merges six record types for the signed-in person (matched by email to their People record, then by owner/responsible-person name): central Actions, NCR (`ncrs` table), Audit Findings (`audit_findings` table, with the parent audit number resolved from `audits`), AINM, Observations, and MOC action-plan items (`moc_action_plan_items`, filtered by `responsible_person`, with the parent MOC number/title resolved from `moc_reports`). Each row carries a `source: "Action" | "NCR" | "Audit Finding" | "AINM" | "Observation" | "MOC"` Type badge; every non-Action source links out via its own deep-link (`/ncr-capa?ncrId=`, `/audits?findingId=`, `/hse/ainm?ainmId=`, `/hse/observations?observationId=`, `/moc?search=<report number>`) rather than opening in the central register. This is a deliberate exception to the normal module-permission boundary — a person's own record ownership is visible in My Actions even without that module's read access, because the point is personal accountability visibility.
- A new "Today" tab sits alongside My Actions on the same page/data (`activeView === "today"` in `app/actions/page.tsx`), reusing the full merged `myActionList` rather than re-fetching. It buckets open items into Overdue / Due Today / Due This Week (days 1-7) / Later-or-no-date (`myTodayOverdueItems`/`myTodayDueTodayItems`/`myTodayDueThisWeekItems`/`myTodayLaterItems`), each rendered as its own urgency-coloured section (`TodayBucketSection`) instead of a flat table — approved via an Artifact mockup ("Today Digest Concept") before implementation, per the standing artifact-first UI rule. This is the first of three "next level" features requested together (Today digest → Notifications → Dark mode); Notifications and Dark mode are not yet started.
