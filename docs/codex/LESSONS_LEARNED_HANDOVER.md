...
- Full-document review and unrestricted repository questioning have separate evidence budgets: free-text questions may deeply analyse up to 250 relevance-ranked candidates, while document reviews analyse the strongest 40 candidates alongside up to 180,000 extracted document characters. Upload/extraction and LL comparison run as separate server requests, with extracted text staged temporarily in secured storage, so large procedures do not combine two long workloads in one function invocation. The UI must translate non-JSON proxy/timeout responses into a readable operational error rather than exposing `<!DOCTYPE>` parsing text.
- The extraction route loads Mammoth and PDFParse only for their matching file types. `next.config.ts` must retain the `/api/lessons-learned/procedure-extract` output-file tracing entries for the PDF.js workers; omitting these can cause a production-only HTTP 500 before route-level error handling, and a PDF import must never be allowed to prevent DOCX extraction from starting.
- AI-native extraction must upload fallback documents to OpenAI Files with purpose `user_data`, pass the returned `file_id` to the Responses API, and delete the OpenAI file in `finally`. Do not send PDFs as inline `file_data`; the production API rejected that path with `Invalid input[0].content[1].file_data`.
- Procedure review must render the same evidence-linked caution cards as ordinary Prevention Intelligence. Every procedure caution requires historic LL support. The model is instructed to return database UUIDs, while server post-processing also resolves returned `LL-xxxxx` lesson numbers back to their candidate UUIDs; invalid references are rejected rather than silently producing an uncited generic review.
- Supporting-lesson drill-downs must preserve the current Prevention Intelligence question, uploaded-document selection and generated brief. The cited Register dataset provides `Back to Prevention Brief`, which restores the prior Intelligence scroll position without rerunning the AI analysis.
