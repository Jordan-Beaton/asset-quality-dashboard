...
- People Management now has a Document-Control-style KPI/dashboard header and a clickable department breakdown, and opens the person detail/edit view as a modal overlay instead of a bottom-anchored panel. Fixed a bug where a blank department silently defaulted to "Assets" on the edit form. See `PEOPLE_MANAGEMENT_HANDOVER.md`.
