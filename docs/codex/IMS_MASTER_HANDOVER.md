...
- New "Overall Inspections" module (`/inspections`) gives a business-wide, project-grouped 8-week inspection lookahead across every project (not just Wadden Sea/Baltic Power), combining live ITP/NOI data with a new manual-entry path for inspections that have no ITP behind them. Wadden Sea's existing dashboard and Project Reports are untouched — Overall Inspections only reads their NOI data. Requires `scripts/sql/project_inspections.sql` to be applied in Supabase before attendees/manual entries work; the page degrades gracefully (ITP/NOI rows only) until then. See `PROJECT_MANAGEMENT_HANDOVER.md`.
- People Management now uses a persistent split-view layout (filterable list on the left, detail pane on the right that updates instantly on selection) instead of a bottom-anchored panel requiring scroll; "Add Person" opens in the same pane. Fixed a bug where a blank department silently defaulted to "Assets" on the edit form. See `PEOPLE_MANAGEMENT_HANDOVER.md`.
- Fixed the public "Request access" email field on the login page validating every address as invalid (not just ones with punctuation in the local part) due to a doubled-backslash regex typo in its `pattern` attribute, present since the feature was added on 4 Aug 2026. See `ADMIN_SETTINGS_HANDOVER.md`.
- Admin/Settings' Users & Access tab was simplified to a "Command Center" layout (KPI strip, horizontal Pending Access Requests row, 2-column permission-editor grid), and the Pending Access Requests panel now always renders with an empty state instead of disappearing when the queue is empty. Three alternate layouts were mocked up as Artifacts and reviewed before implementation. See `ADMIN_SETTINGS_HANDOVER.md`.
- Central Action Management gained a new "Today" tab next to My Actions, bucketing every item assigned to the signed-in person (now including MOC action-plan items) into Overdue / Due Today / Due This Week / Later. First of three "next level" features requested together (Today digest → Notifications → Dark mode, in that order); approved via Artifact mockup before implementation. See `ACTION_MANAGEMENT_HANDOVER.md`.
- Fixed Document Control's Details tab clipping the labels/values of its paired fields (Document Type/Department, Current Revision/Review Cycle, Issue Date/Next Review Date) at narrower widths — three grid rows were missing the `minmax(0, 1fr)` guard used everywhere else in the file. See `DOCUMENT_CONTROL_HANDOVER.md`.
- IMS Home's view lineup was revised on request: Spotlight and List (as a selectable desktop view) and Two columns removed as unused; Mission Control, Constellation, and Holo Deck added, each first approved as an Artifact mockup with ambient motion before being built for real with the same permission gating as every other view. See `PROJECT_ROADMAP.md`'s "IMS Home / Launchpad" section.
- Added a Notification Centre: a bell icon in the shared header (`src/components/AppShell.tsx`) opens a panel (`src/components/NotificationBell.tsx`) backed by a new generic `ims_notifications` table, the app's first cross-feature notification log. Written alongside existing emails at assignment, document workflow, ITP sign-off, and new-access-request touchpoints via `src/lib/notifications.ts`. Requires `scripts/sql/ims_notifications.sql` to be applied in Supabase; degrades gracefully (empty bell, no errors) until then. See `PROJECT_ROADMAP.md`'s "Notification Centre" section.
- Added Dark Mode: a toggle in the header next to the notification bell, persisted to `localStorage`, driving a `data-theme` attribute on `.ims-app-root`. `UI_STANDARDS.md`'s prior ban on dark themes was amended with an explicit exception for this user-toggled whole-app theme. `src/components/imsTheme.ts`'s shared colour tokens now resolve via CSS custom properties (`app/globals.css`), so anything built from them inverts automatically; the shared shell (`AppShell.tsx`) was retrofitted directly. Per-module retrofit beyond that is a large incremental effort, not yet started. See `PROJECT_ROADMAP.md`'s "Dark Mode" section.
- Fixed two "Clear Filters" buttons rendering oversized/full-width next to their "Show Filters" toggle: `app/actions/page.tsx`'s My Actions tab and `app/hse/ptw/page.tsx`'s PTW register expanded filters were both reusing a fixed-column-count CSS grid style designed for a different number of children (a search input plus two buttons), so a button landed in the wide column meant for that input. Found by a targeted codebase-wide scan for the same class of bug after the user spotted one instance live; no other instances found. Fixed each with its own correctly-sized layout rather than the mismatched shared style.
