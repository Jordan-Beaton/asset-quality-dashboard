...
- IMS Home is now a dedicated permission-aware launchpad with a compact Enshore dark-blue animated header, synchronized official outline-logo video treatments, equal-size workspace cards, pronounced hover interactions, and six selectable access layouts: Card grid, Compact tiles, IMS hub, Mission Control, Constellation, and Holo Deck. The selected layout persists locally between browser sessions.
...
- People Management revamped with a persistent split-view layout — a filterable list on the left, a detail pane on the right that updates instantly on selection, no modal and no scrolling to a panel at the bottom of the page. Three alternate layouts were mocked up as Artifacts and reviewed before implementation. Fixed a bug where a person with a blank department silently defaulted to "Assets" in the edit form, risking an incorrect overwrite on save; the department field is now genuinely optional throughout. See `PEOPLE_MANAGEMENT_HANDOVER.md`.
- Fixed the public "Request access" form on the login page rejecting every email address (not just ones with punctuation in the local part) as an invalid format, due to a doubled-backslash typo in the email field's regex `pattern` attribute that has been present since the feature was added on 4 Aug 2026. See `ADMIN_SETTINGS_HANDOVER.md`.
- Resolved the invite/reset email rate-limit: Supabase Auth was using its default shared email sender (~2/hour limit), previously mis-documented as a Resend limit. Configured custom SMTP in the Supabase Dashboard pointed at Resend, confirmed working with a live test email. See `ADMIN_SETTINGS_HANDOVER.md`.
- Admin / Settings "Command Center" layout: the Users & Access tab now opens with a KPI strip (IMS Users/Active/Pending Invites/Admins/Departments/Projects), Pending Access Requests as a horizontal card row, and per-person/invite permission editors laid out as a 2-column grid of module cards instead of one long vertical list. Three alternate layouts were mocked up as Artifacts and reviewed before implementation; this is a layout-only change — every access level, checkbox and dropdown behaves exactly as before. See `ADMIN_SETTINGS_HANDOVER.md`.
...
- Summary: IMS Home is intentionally an access surface rather than a dashboard. It uses a compact Enshore dark-blue animated header and equal-size permission-aware workspace cards. Users can choose Card grid, Compact tiles, IMS hub, Mission Control, Constellation, or Holo Deck. Spotlight, List (as a selectable desktop view), and Two columns were removed on request (unused); List is retained internally only as the forced simple layout on phone widths, matching the permanent phone-list rule below. Mission Control (dark HUD console with a live clock, scanline sweep, drifting grid, and pulsing status dots), Constellation (module groups laid out as clusters around a central "IMS" node with animated flowing connection lines), and Holo Deck (glassmorphic tilting cards over an aurora/rising-particle background) were each designed as an Artifact mockup, approved by the user, then implemented for real in `app/home/page.tsx` reusing the same `moduleCards` data and `isModuleAccessible` permission gating as every other view — restricted modules render dimmed/non-clickable exactly as they do in Card grid/Compact tiles/IMS hub. The IMS hub enlarges the official outline animation and positions compact workspace segments around it. The header and hub videos remain synchronized while both are visible, and the last selected view is restored from browser local storage (an old saved preference of `spotlight`/`list`/`columns` simply falls back to Card grid since those ids are no longer in the selectable list).
- Permanent Rules:
  ...
  - Mission Control, Constellation, and Holo Deck use only the approved 2026 Enshore palette (`#005670`, `#63B1BC`, `#FFAD00`, `#000000`, `#FFFFFF`, `#D0D0CE`, `#53565A`) via rgba()/gradient blends for their darker HUD/aurora backgrounds — no restricted green/purple accents and no new hex literals, to keep `npm run check:ui` passing.
- Outstanding Actions:
  - Verify all six views on Vercel at desktop, tablet, and mobile widths, including Mission Control/Constellation/Holo Deck specifically (not covered by prior production QA passes).
...
- Status: Complete
- Summary: People Management is the source for person records and IMS dropdown values. Excel import exists, generated email logic exists, and the module is separate from Admin login users. The page now uses a persistent split-view layout (filterable list left, detail pane right, updates instantly on selection) instead of an inline panel requiring scroll. Fixed a bug where a blank department silently defaulted to "Assets" in the edit form.
- Outstanding Actions:
