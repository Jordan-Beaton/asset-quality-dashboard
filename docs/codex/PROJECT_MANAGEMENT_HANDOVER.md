...
- Data model:
  ...
  - `project_manual_inspections` (new, same SQL file) — free-standing manual inspection entries. `project_key`/`project_label` are plain text (not tied to `projectRegistry.ts`), so a brand-new project can be logged here immediately without a code change. Includes `attendees` (jsonb), `client_visible`, and internal-only `notes` (never exported).
  - **Apply `scripts/sql/project_inspections.sql` in the Supabase SQL editor before relying on attendees or manual entries** — until it's applied, the page still loads and shows ITP/NOI-derived rows only, with a status message pointing at the SQL file.
...
- NOI/ITP-derived rows are read-only here (edit via the existing NOI Creator — the detail panel links straight to `/projects/{key}/noi/create?noi=...`); only manual entries can be created/edited/deleted from this page, gated by the same `canCreate`/`canEdit` permission flags as the rest of Action/Project Management.
