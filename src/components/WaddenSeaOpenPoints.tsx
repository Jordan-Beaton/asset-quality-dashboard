...
export function WaddenSeaOpenPoints() {
  ...
  async function exportWord(rows = filtered, closureOnly = false) {
    ...
    const makeCell = (value: string, header = false, fill?: string) => new TableCell({ margins: { top: 90, bottom: 90, left: 100, right: 100 }, shading: header ? { fill: "005670" } : fill ? { fill } : undefined, children: [new Paragraph({ children: [new TextRun({ text: value || "-", font: "Arial", bold: header, color: header ? "FFFFFF" : "0F172A", size: header ? 16 : 15 })] })] });
...
function Badge({ text, danger }: { text: string; danger?: boolean }) { return <span style={{ ...badge, color: danger ? "#F93822" : "#334155", background: danger ? "#fee2e2" : "#e2e8f0" }}>{text}</span>; }
