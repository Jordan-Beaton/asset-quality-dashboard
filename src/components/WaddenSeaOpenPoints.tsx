...
export function WaddenSeaOpenPoints() {
  ...
  async function exportWord(rows = filtered, closureOnly = false) {
    ...
    const makeCell = (value: string, header = false, fill?: string) => new TableCell({ margins: { top: 90, bottom: 90, left: 100, right: 100 }, shading: header ? { fill: "005670" } : fill ? { fill } : undefined, children: [new Paragraph({ children: [new TextRun({ text: value || "-", font: "Calibri", bold: header, color: header ? "FFFFFF" : "000000", size: header ? 16 : 15 })] })] });
    ...
    const doc = new WordDocument({ styles: { default: { document: { run: { font: "Calibri", size: 17, color: "000000" } } } }, sections: [{ properties: { page: { size: { orientation: PageOrientation.LANDSCAPE }, margin: { top: 540, right: 540, bottom: 720, left: 540, footer: 300 } } }, footers: { default: new Footer({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `Wadden Sea | ${title} | Page `, font: "Calibri", size: 15, color: "53565A" }), new SimpleField("PAGE"), new TextRun({ text: " of ", font: "Calibri", size: 15, color: "53565A" }), new SimpleField("NUMPAGES")] })] }) }, children: [new Paragraph({ spacing: { after: 50 }, children: [new TextRun({ text: "WADDEN SEA PROJECT", font: "Calibri", bold: true, size: 32, color: "005670" })] }), new Paragraph({ spacing: { after: 70 }, children: [new TextRun({ text: title, font: "Calibri", bold: true, size: 26 })] }), new Paragraph({ spacing: { after: 170 }, children: [new TextRun({ text: `${rows.length} controlled record${rows.length === 1 ? "" : "s"} | Generated ${new Date().toLocaleString("en-GB")}`, font: "Calibri", size: 16, color: "53565A" })] }), ...body] }] });
