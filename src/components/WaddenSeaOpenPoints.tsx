...
export function WaddenSeaOpenPoints() {
  ...
  async function openEvidence(item: Evidence) { const { data, error } = await supabase.storage.from("project-documents").createSignedUrl(item.file_path, 60 * 60 * 24 * 180); if (error || !data?.signedUrl) { setMessage(error?.message || "Evidence could not be opened."); return; } window.open(data.signedUrl, "_blank", "noopener,noreferrer"); }
  ...
  function exportPdf() {
    ...
    doc.setFillColor(0, 86, 112); doc.rect(0, 0, width, 24, "F"); doc.setFont("helvetica", "bold"); doc.setTextColor(255, 255, 255); doc.setFontSize(17); doc.text("WADDEN SEA PROJECT", 10, 11); doc.setFontSize(10); doc.text("Open Points Register", 10, 18);
    doc.setTextColor(83, 86, 90); doc.setFont("helvetica", "normal"); doc.setFontSize(8); doc.text(`Generated ${new Date().toLocaleString("en-GB")} | ${filtered.length} controlled records`, 10, 29);
    autoTable(doc, { startY: 33, theme: "grid", margin: { left: 10, right: 10, bottom: 15 }, head: [["ID", "Title / Description", "Severity", "Status", "Owner", "Phase", "Due", "NCR"]], body: filtered.map((p) => [p.open_point_number, `${p.title}\n${p.description}`, p.severity, p.status, p.owner || "-", p.project_phase || "-", fmt(effectiveDue(p)), p.ncr_reference || "-"]), headStyles: { fillColor: [0, 86, 112], textColor: [255, 255, 255], fontStyle: "bold" }, styles: { fontSize: 7.5, cellPadding: 2, textColor: [0, 0, 0], lineColor: [208, 208, 206], lineWidth: 0.2, valign: "middle", overflow: "linebreak" }, alternateRowStyles: { fillColor: [236, 236, 231] }, columnStyles: { 1: { cellWidth: 85 } } });
    const pages = doc.getNumberOfPages(); for (let page = 1; page <= pages; page += 1) { doc.setPage(page); doc.setTextColor(83, 86, 90); doc.setFontSize(8); doc.text("Wadden Sea | Open Points Register", 10, height - 6); doc.text(`Page ${page} of ${pages}`, width - 10, height - 6, { align: "right" }); } doc.save(`Wadden-Sea-Open-Points-${new Date().toISOString().slice(0, 10)}.pdf`);
  }
  async function exportWord(rows = filtered, closureOnly = false) {
    const borders = { top: { style: BorderStyle.SINGLE, size: 1, color: "D0D0CE" }, bottom: { style: BorderStyle.SINGLE, size: 1, color: "D0D0CE" }, left: { style: BorderStyle.SINGLE, size: 1, color: "D0D0CE" }, right: { style: BorderStyle.SINGLE, size: 1, color: "D0D0CE" }, insideHorizontal: { style: BorderStyle.SINGLE, size: 1, color: "D0D0CE" }, insideVertical: { style: BorderStyle.SINGLE, size: 1, color: "D0D0CE" } };
    const makeCell = (value: string, header = false, fill?: string) => new TableCell({ margins: { top: 90, bottom: 90, left: 100, right: 100 }, shading: header ? { fill: "005670" } : fill ? { fill } : undefined, children: [new Paragraph({ children: [new TextRun({ text: value || "-", font: "Azo Sans", bold: header, color: header ? "FFFFFF" : "000000", size: header ? 16 : 15 })] })] });
    const title = closureOnly && rows[0] ? `${rows[0].open_point_number} - Closure Report` : "Open Points Register";
    const registerTable = new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, layout: TableLayoutType.FIXED, borders, columnWidths: [900, 3300, 900, 1050, 1200, 1050, 850, 900], rows: [new TableRow({ tableHeader: true, children: ["ID", "Title / Description", "Severity", "Status", "Owner", "Phase", "Due", "NCR"].map((value) => makeCell(value, true)) }), ...rows.map((p, index) => new TableRow({ cantSplit: true, children: [p.open_point_number, `${p.title}\n${p.description}`, p.severity, p.status, p.owner || "-", p.project_phase || "-", fmt(effectiveDue(p)), p.ncr_reference || "-"].map((value) => makeCell(value, false, index % 2 === 0 ? "ECECE7" : undefined)) }))] });
    const closureContent = rows[0] ? [["Outstanding Work", rows[0].description], ["Severity / Status", `${rows[0].severity} / ${rows[0].status}`], ["Owner", rows[0].owner || "-"], ["Linked NCR", rows[0].ncr_reference || "-"], ["Resolution", rows[0].resolution_action || "-"], ["Verification", `${rows[0].verification_method || "-"} | ${rows[0].verified_by || "-"} | ${fmt(rows[0].verified_at)}`], ["Client Verification", rows[0].employer_verification_status], ["Closure", `${fmt(rows[0].closure_date)} | ${rows[0].closure_report_reference || "-"}`]].map(([label, value], index) => new TableRow({ children: [makeCell(label, false, index % 2 === 0 ? "ECECE7" : undefined), makeCell(value, false, index % 2 === 0 ? "ECECE7" : undefined)] })) : [];
    ...
    const doc = new WordDocument({ styles: { default: { document: { run: { font: "Azo Sans", size: 17, color: "000000" } } } }, sections: [{ properties: { page: { size: { orientation: PageOrientation.LANDSCAPE }, margin: { top: 540, right: 540, bottom: 720, left: 540, footer: 300 } } }, footers: { default: new Footer({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `Wadden Sea | ${title} | Page `, font: "Azo Sans", size: 15, color: "53565A" }), new SimpleField("PAGE"), new TextRun({ text: " of ", font: "Azo Sans", size: 15, color: "53565A" }), new SimpleField("NUMPAGES")] })] }) }, children: [new Paragraph({ spacing: { after: 50 }, children: [new TextRun({ text: "WADDEN SEA PROJECT", font: "Azo Sans", bold: true, size: 32, color: "005670" })] }), new Paragraph({ spacing: { after: 70 }, children: [new TextRun({ text: title, font: "Azo Sans", bold: true, size: 26 })] }), new Paragraph({ spacing: { after: 170 }, children: [new TextRun({ text: `${rows.length} controlled record${rows.length === 1 ? "" : "s"} | Generated ${new Date().toLocaleString("en-GB")}`, font: "Azo Sans", size: 16, color: "53565A" })] }), ...body] }] });
  ...
  return <section style={shell}>
    <nav className="ims-tabs" style={tabs} role="tablist" aria-label="Open Point views">{(["dashboard", "register", "create", "settings"] as View[]).map((item) => <button key={item} role="tab" aria-selected={view === item} data-active={view === item ? "true" : "false"} style={view === item ? activeTab : tab} onClick={() => item === "create" ? resetForm() : setView(item)}>{item === "create" ? "Raise Open Point" : item[0].toUpperCase() + item.slice(1)}</button>)}</nav>
    <div style={statusBar}><strong>Open Points:</strong> {message}<span>Wadden Sea controlled register</span></div>
    {view === "dashboard" && <><div style={kpis}><QualityKpiCard title="Total Open" value={metrics.open} accent="#005670" onClick={() => { setStatusFilter("All"); setView("register"); }} /><QualityKpiCard title="Critical" value={metrics.critical} accent="#F93822" onClick={() => { setSeverityFilter("Critical"); setView("register"); }} /><QualityKpiCard title="Major" value={metrics.major} accent="#FFAD00" onClick={() => { setSeverityFilter("Major"); setView("register"); }} /><QualityKpiCard title="Minor" value={metrics.minor} accent="#63B1BC" onClick={() => { setSeverityFilter("Minor"); setView("register"); }} /><QualityKpiCard title="Overdue" value={metrics.overdue} accent="#F93822" onClick={() => { setShowOnlyOverdue(true); setView("register"); }} /><QualityKpiCard title="Employer Raised" value={metrics.employer} accent="#53565A" onClick={() => { setSourceFilter("Employer"); setView("register"); }} /><QualityKpiCard title="Awaiting Verification" value={metrics.verification} accent="#000000" onClick={() => setView("register")} /><QualityKpiCard title="CDE Mirror Due" value={metrics.mirror} accent="#FFAD00" onClick={() => setView("register")} /></div><div style={twoCol}><Panel title="ADM Compliance Monitor"><Compliance label="Electronic register" ok /><Compliance label="Weekly CDE visibility" ok={metrics.mirror === 0} detail={`${metrics.mirror} require mirroring`} /><Compliance label="Employer points registered in 7 days" ok={!points.some((p) => p.source_type === "Employer" && !p.cde_registered_at && Boolean(p.cde_registration_due) && p.cde_registration_due! < new Date().toISOString().slice(0, 10))} /><Compliance label="Severity assigned" ok={!points.some((p) => !p.severity)} /><Compliance label="Closure evidence and verification" ok={!points.some((p) => isClosed(p) && !evidence.some((e) => e.open_point_id === p.id && e.evidence_type !== "Supporting Evidence"))} /></Panel><Panel title="Immediate Attention"><div style={attention}>{points.filter((p) => isOverdue(p) || mirrorOverdue(p)).slice(0, 12).map((p) => <button key={p.id} style={attentionRow} onClick={() => openPoint(p)}><span><strong>{p.open_point_number}</strong><small>{p.title}</small></span><b>{isOverdue(p) ? "Overdue" : "CDE due"}</b></button>)}{!points.length && <p>No Open Points recorded.</p>}</div></Panel></div></>}
    ...
    >
      ...
      <div style={tableWrap}><table style={table}><colgroup>{[115, 330, 100, 150, 180, 155, 125, 120, 95].map((x, i) => <col key={i} style={{ width: x }} />)}</colgroup><thead><tr>{["ID", "Open Point", "Severity", "Status", "Owner / Company", "Phase", "Due", "Client Copy", "Evidence"].map((x) => <th key={x} style={tableHeader}>{x}</th>)}</tr></thead><tbody>{filtered.map((p) => <tr key={p.id} onClick={() => openPoint(p)} style={tableRow}><td style={tableCell}><strong>{p.open_point_number}</strong></td><td style={tableCell}><strong>{p.title}</strong><small style={cellDetail}>{p.description}</small></td><td style={tableCell}><Badge text={p.severity} danger={p.severity === "Critical"} /></td><td style={tableCell}>{p.status}</td><td style={tableCell}>{p.owner || "-"}<small style={cellDetail}>{p.responsible_company}</small></td><td style={tableCell}>{p.project_phase || "-"}</td><td style={{ ...tableCell, color: isOverdue(p) ? "#F93822" : undefined, fontWeight: isOverdue(p) ? 800 : undefined }}>{fmt(effectiveDue(p))}</td><td style={tableCell}><button style={linkButton} onClick={(e) => { e.stopPropagation(); void markMirrored(p); }}>{mirrorOverdue(p) ? "Copy due" : fmt(p.cde_mirrored_at)}</button></td><td style={tableCell}>{evidence.filter((x) => x.open_point_id === p.id).length}</td></tr>)}</tbody></table></div>
...
function Badge({ text, danger }: { text: string; danger?: boolean }) { return <span style={{ ...badge, color: danger ? "#F93822" : "#53565A", background: danger ? "#ECECE7" : "#D0D0CE" }}>{text}</span>; }

const shell: CSSProperties = { display: "grid", gap: 16 };
const tabs: CSSProperties = { display: "flex", flexWrap: "wrap", gap: 8, padding: 6, borderRadius: 14, background: "#D0D0CE" };
const tab: CSSProperties = { border: 0, borderRadius: 10, padding: "10px 15px", background: "transparent", color: "#53565A", fontWeight: 800, cursor: "pointer" };
const activeTab: CSSProperties = { ...tab, color: "#ffffff", background: "#005670" };
const statusBar: CSSProperties = { display: "flex", flexWrap: "wrap", justifyContent: "space-between", gap: 12, padding: "11px 14px", border: "1px solid #D0D0CE", borderRadius: 12, background: "#ECECE7", color: "#005670", fontSize: 13 };
...
const panel: CSSProperties = { minWidth: 0, overflow: "hidden", padding: 20, background: "#ffffff", border: "1px solid #D0D0CE", borderRadius: 16, boxShadow: "0 8px 22px rgba(15,23,42,.05)" };
const compliance: CSSProperties = { display: "grid", gridTemplateColumns: "28px 1fr auto", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid #D0D0CE" };
const attention: CSSProperties = { display: "grid", gap: 8 };
const attentionRow: CSSProperties = { display: "flex", justifyContent: "space-between", gap: 12, padding: 12, border: "1px solid #ECECE7", borderRadius: 10, background: "#ECECE7", textAlign: "left", cursor: "pointer" };
...
const input: CSSProperties = { width: "100%", minWidth: 0, maxWidth: "100%", minHeight: 40, padding: "9px 11px", border: "1px solid #D0D0CE", borderRadius: 9, background: "#fff", color: "#000000", font: "inherit", boxSizing: "border-box" };
...
const field: CSSProperties = { display: "grid", minWidth: 0, gap: 6, fontSize: 12, fontWeight: 800, color: "#53565A" };
const formHelp: CSSProperties = { margin: "0 0 14px", color: "#53565A", fontSize: 13, lineHeight: 1.5 };
const detailsPanel: CSSProperties = { gridColumn: "1 / -1", minWidth: 0, padding: 14, border: "1px solid #D0D0CE", borderRadius: 12, background: "#ECECE7" };
const detailsSummary: CSSProperties = { cursor: "pointer", color: "#005670", fontSize: 14, fontWeight: 900, marginBottom: 12 };
...
const button: CSSProperties = { border: "1px solid #D0D0CE", borderRadius: 9, padding: "10px 14px", background: "#D0D0CE", color: "#000000", fontWeight: 800, cursor: "pointer" };
const miniButton: CSSProperties = { ...button, minHeight: 40, padding: "8px 11px", whiteSpace: "nowrap" };
const primary: CSSProperties = { ...button, borderColor: "#005670", background: "#005670", color: "#ffffff" };
const danger: CSSProperties = { ...button, borderColor: "#F93822", background: "#F93822", color: "#ffffff" };
const ghost: CSSProperties = { ...button, background: "#ffffff" };
const linkButton: CSSProperties = { border: 0, background: "transparent", color: "#005670", fontWeight: 800, cursor: "pointer", padding: 0 };
...
const tableHeader: CSSProperties = { padding: "12px 14px", background: "#ECECE7", borderBottom: "1px solid #D0D0CE", color: "#53565A", fontSize: 12, fontWeight: 900, letterSpacing: "0.04em", textAlign: "left", textTransform: "uppercase" };
const tableCell: CSSProperties = { padding: "12px 14px", borderBottom: "1px solid #ECECE7", color: "#000000", lineHeight: 1.45, verticalAlign: "top", overflowWrap: "anywhere" };
const tableRow: CSSProperties = { cursor: "pointer" };
const cellDetail: CSSProperties = { display: "block", marginTop: 3, color: "#53565A", fontSize: 12, fontWeight: 400, lineHeight: 1.4 };
const registerCount: CSSProperties = { margin: "12px 0", color: "#53565A", fontSize: 13, fontWeight: 700 };
const compactActions: CSSProperties = { display: "flex", flexWrap: "wrap", gap: 8 };
const emptyRegister: CSSProperties = { padding: 24, border: "1px dashed #D0D0CE", borderRadius: 12, color: "#53565A", textAlign: "center" };
const badge: CSSProperties = { display: "inline-block", padding: "4px 7px", borderRadius: 999, fontWeight: 800, fontSize: 11 };
const evidencePanel: CSSProperties = { marginTop: 20, paddingTop: 14, borderTop: "1px solid #D0D0CE" };
const evidenceList: CSSProperties = { display: "grid", gap: 8 };
const evidenceRow: CSSProperties = { display: "grid", gridTemplateColumns: "180px 1fr auto", gap: 10, padding: 11, border: "1px solid #D0D0CE", borderRadius: 10, background: "#ECECE7", textAlign: "left", cursor: "pointer" };
