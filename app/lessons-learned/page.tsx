...
export default function LessonsLearnedPage() {
  ...
  const [form, setForm] = useState(emptyForm); const [message, setMessage] = useState("Loading lessons learnt...");
  ...
  async function loadData() {
    ...
    try {
      ...
      setMessage(`Lessons Learnt load failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function refreshLessonIndex(recordIds: string[]) {
    if (!recordIds.length) return;
    try {
      const response = await fetch("/api/lessons-learned/prevention-index", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lessonIds: recordIds.slice(0, 20) }) });
      if (!response.ok) console.warn("Automatic lesson indexing was deferred.");
    } catch { console.warn("Automatic lesson indexing was deferred."); }
  }
  async function saveLesson() {
    if (selected ? !requireEdit("update lessons") : !requireCreate("create lessons")) return;
    if (!form.project_name.trim() || !form.subject.trim() || !form.issue_description.trim() || !form.lesson_learned.trim()) { setMessage("Project, subject, what happened, and lesson learnt are required."); return; }
    ...
    await refreshLessonIndex([recordId]);
  ...
  async function runImport() {
    ...
    for (let index = 0; index < valid.length; index += 200) { const batch = valid.slice(index, index + 200).map(({ rowNumber, errors, warnings, ...row }) => row); const { data, error } = await supabase.from("lessons_learned").insert(batch).select("id,outcome_type"); if (error) { setMessage(`Import stopped after ${imported.toLocaleString()} rows: ${error.message}`); setImporting(false); return; } imported += batch.length; const failureIds = (data || []).filter((row) => row.outcome_type === "Failure").map((row) => row.id); for (let offset = 0; offset < failureIds.length; offset += 20) await refreshLessonIndex(failureIds.slice(offset, offset + 20)); setMessage(`Importing and indexing ${imported.toLocaleString()} of ${valid.length.toLocaleString()} lessons...`); }
  ...
  return <ReferenceContext.Provider value={{ projects: referenceProjects, people: peopleOptions, assets: assetOptions, departments: departmentOptions, onSelectProject: selectProjectReference, showNewProject, setShowNewProject, newProjectCode, setNewProjectCode, newProjectName, setNewProjectName, onAddProject: addProjectReference }}><main className={isFieldMode ? "lessons-field-mode" : undefined}>
    {isFieldMode ? <section style={fieldIntroStyle}><span style={fieldIntroIconStyle}><LearningFieldIcon /></span><span><strong style={fieldIntroTitleStyle}>Capture a Lesson</strong><small style={fieldIntroTextStyle}>Record the learning now; optional detail can be added or refined later.</small></span></section> : <QualityPageHero label="LESSONS LEARNT" title="Lessons Learnt" description="Central repository for searchable project knowledge, repeat-failure prevention, evidence, actions, and trend analysis." />}
    ...
    {!isFieldMode ? <ImsTabs tabs={tabs} active={view} onChange={(next) => { setView(next); setIntelligenceDrilldown(false); if (next === "create") { setSelected(null); setForm(emptyForm); } }} ariaLabel="Lessons Learnt views" /> : null}
    ...
    {view === "register" && <>
      <ImsPanel title="Lessons Learnt Register" subtitle="Search the full knowledge base by project, department, subject, issue, lesson, root cause, owner, or keyword.">
    ...
    {view === "import" && <ImsPanel title="Master Lessons Learnt Excel Import" subtitle="Maps the supplied 18-column master workbook into the central repository. Existing project/subject/issue matches are flagged before import.">
...
function LessonForm({ title, subtitle, form, update, files, setFiles, evidenceNotes, setEvidenceNotes, saving, onSave, onDelete, onClose, evidence, onOpenEvidence, fieldMode = false }: { title: string; subtitle: string; form: typeof emptyForm; update: (name: keyof typeof emptyForm, value: string) => void; files: File[]; setFiles: (files: File[]) => void; evidenceNotes: string; setEvidenceNotes: (value: string) => void; saving: boolean; onSave: () => void; onDelete?: () => void; onClose?: () => void; evidence: Evidence[]; onOpenEvidence: (item: Evidence) => void; fieldMode?: boolean }) {
  ...
  if (fieldMode) return <ImsPanel title={title} subtitle={subtitle}>
    <div style={fieldFormGrid}>
      ...
      <Field label="Lesson Learnt"><Textarea value={form.lesson_learned} onChange={(x) => update("lesson_learned", x)} placeholder="What should another team know or repeat?" /></Field>
  ...
  return <ImsPanel title={title} subtitle={subtitle}><div style={formGrid}>
    ...
    <Field label="Action Taken" wide><Textarea value={form.action_taken} onChange={(x) => update("action_taken", x)} /></Field><Field label="Lesson Learnt" wide><Textarea value={form.lesson_learned} onChange={(x) => update("lesson_learned", x)} /></Field>
