...
export default function HsePermitToWorkPage() {
  ...
  async function createSignedAttachmentLinks() {
    ...
    // emailed to external recipients with no IMS access — 6 months (180 days)
    // so the link itself carries the access, rather than a short-lived,
    // in-app-only expiry.
    const rows = await Promise.all(
      selectedAttachments.map(async (attachment) => {
        const { data } = await supabase.storage.from(evidenceBucket).createSignedUrl(attachment.file_path, 60 * 60 * 24 * 180);
        return {
