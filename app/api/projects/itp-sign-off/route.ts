...
function projectDisplayName(key: string) {
  if (key === "wadden-sea") return "Wadden Sea";
  if (key === "baltic-power") return "Baltic Power";
...
export async function PUT(request: Request) {
  try {
    ...
    const { resend, from } = emailClient();
    const result = await resend.emails.send({
      from, to: [clean(signoff.recipient_email)], subject: `Your ${projectDisplayName(clean(signoff.project_key))} ITP sign-off verification code`,
...
export async function PATCH(request: Request) {
  try {
    ...
    const certificate = createItpSignOffCertificate({
      requestId: clean(signoff.id), projectName: projectDisplayName(clean(signoff.project_key)), documentName: clean(signoff.document_name), documentPath: clean(signoff.document_path),
    ...
    const sourceFolder = sourcePath.includes("/") ? sourcePath.slice(0, sourcePath.lastIndexOf("/")) : `${clean(signoff.project_key)}/itp-sign-offs/${clean(signoff.id)}`;
